...
class Encoder(nn.Module):
    ...
    def __init__(self, obs_shape, cfg):
        ...
        # First conv layer with stride=2
        self.convs = nn.ModuleList([nn.Conv2d(obs_shape[0], self.num_filters, 3, stride=2, padding=1)])
        
        # Subsequent layers with stride=1 and padding=1 to maintain spatial dimensions
        for i in range(1, self.num_layers):
            self.convs.append(nn.Conv2d(self.num_filters, self.num_filters, 3, stride=1, padding=1))

        # Calculate output shape by running a forward pass
        with torch.no_grad():
            x = torch.randn(1, *obs_shape)  # Create dummy input with correct shape
