...
class Dreamer(nn.Module):
  ...
  def __init__(self, config, logger, dataset):
    ...
    elif config.ted_simplified:
      self._wm = models.WorldModel_TED_Simplified(self._step, config)
    else:
...
def main(config):
  ...
  if isinstance(config.traindir, str):
    config.traindir = pathlib.Path(config.traindir)
  if isinstance(config.evaldir, str):
    config.evaldir = pathlib.Path(config.evaldir)
